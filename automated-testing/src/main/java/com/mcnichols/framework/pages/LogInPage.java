...
public class LogInPage {
	...
	public void logIn(int accountType) {
		logInAsRegisteredUser(TestingConfig.getLogin(accountType).getUsername(), TestingConfig.getLogin(accountType).getPassword());
	}
	...
	public void logInAsRegisteredUser(String username, String password) {
		...
		if (Browser.isElementPresent(By.cssSelector("button.login-btn"))) {
			...
			emailField = "#accountLoginForm .row input#login"; //ToDo: There is more than one element ID on page.  Needs to be resolved!
			passwordField = "#accountLoginForm .row input#password";
			
			Browser.scrollToElememnt(Browser.getWebElement(By.id("new-account-tab")));
			Browser.getWebElement(By.id("new-account-tab")).click();
			Logger.info("Log In Page - Selected tab new account.");
			Browser.waitForSomeTime();
			Browser.getWebElement(By.id("login-tab")).click();
			Logger.info("Log In Page - Selected tab login.");
			Browser.waitForSomeTime();
			Browser.getWebElement(By.id("login")).click();
			Browser.waitForSomeTime();
		}
		...
		Browser.getWebElement(By.cssSelector(passwordField)).click();
	...
	public void createAccount(int accountType) {
		...
		Logger.info(pageNamePrefixForLogger + "Form Login - Selected Email Address field and entered: " + userEmailAddress);
		Browser.waitForSomeTime();
