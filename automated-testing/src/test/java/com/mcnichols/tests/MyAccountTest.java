...
public class MyAccountTest extends TestClassBase {
	...
	public void overrideCCDetail() throws Exception {
		Pages.accountCreditCardsPage().overrideCCdetail(true);
		verifyTrue(Pages.accountCreditCardsPage().isAt(), "Is at New CreditCard Popup!");
	}
	
	@Test(dependsOnMethods = { "overrideCCDetail" }, description = "Is Credit Card update successful")
	public void verifyCreditCardUpdate() throws Exception {
		boolean isCreditCardUpdateSuccessful = Pages.accountCreditCardsPage().isCreditCardUpadteSuccessful();
		verifyTrue(isCreditCardUpdateSuccessful, "Credit card has been successfully updated.");
	}
}
